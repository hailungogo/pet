...
app.get("/getCount",(req,res)=>{
    ...
    var sql="SELECT SUM(isShoucang) AS count FROM pet_shoucang WHERE uid=?";
...
app.get("/addShoucang",(req,res)=>{
    var did=req.query.did;
    var uid=req.session.obj;
    var isShoucang=req.query.isShoucang;
    if(!uid){
    ...
    pool.query(sql,[uid,did,isShoucang],(err,result)=>{
        if(err) throw err;
        console.log(result);
        if(result.affectedRows>0){
...
//8.3查询收藏记录
 ...
 //8.4查询是否已收藏
 app.get("/checkShoucang",(req,res)=>{
     var uid=req.session.obj;
     var did=req.query.did;
     var sql="SELECT isShoucang FROM pet_shoucang WHERE did=? AND uid=?";
     pool.query(sql,[did,uid],(err,result)=>{
         if(err) throw err;
         if(result.length>0){
             res.send({code:1,data:1})
         }else{
